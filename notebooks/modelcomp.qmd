...
model_cv = tibble::tibble(model = list(lr_res, knn_res, svm_res, rf_res, ann_res, xgboost_res), model_name = c("logistic", "knn", "svm", "rf", "ann", "xgb"))
...
model_cv |>
  dplyr::filter(.metric == "roc_auc") |>
  ggplot2::ggplot(aes(x = model_name, y = .estimate, color = id, group = id)) +
  ggplot2::geom_line() +
...
model_comp |>
  ...
  ggplot2::geom_histogram(bins = 40, col = "blue", fill = "blue", alpha = .4) + 
  ggplot2::facet_wrap(~ model, ncol = 1) + 
  ggplot2::xlab("ROC-AUC Score Posterior Distribution")
```
 
```{r}
ggplot2::autoplot(model_comp)
```
...
```{r}
model_cont = tidyposterior::contrast_models(model_comp, seed = 3145)
...
model_comp |>
  ...
  ggplot2::geom_histogram(bins = 40, col = "blue", fill = "blue", alpha = .4) + 
  ggplot2::facet_wrap(~ model, ncol = 1) + 
  ggplot2::xlab("Accuracy Score Posterior Distribution")
```
