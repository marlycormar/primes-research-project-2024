...
and the quantity is called the "odds" -- as it denotes the estimated probability of recurrence over no recurrence. It follows that the quantity
...
(Note that this maximizes the probability of the observed training data.)

Or equivalently, we want to maximize the *log likelihood function*
...
```{r}
# Read cleaned data.
cleaned_data <- readr::read_csv(here::here('Thyroid_Diff.csv'))
```


```{r lr-data-split, cache=TRUE}
