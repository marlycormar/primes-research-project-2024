...
cleaned_data |>
  ...
  dplyr::reframe(
    total = dplyr::n(),
    total_rec = round(dplyr::n()/total*100, 1), total
  ) |>
  plotly::plot_ly(
    x = ~Adenopathy,
    y = ~total_rec,
...
<!--     opacity = 0.7, -->
<!--     type = 'bar', -->
<!--     hovertemplate = '<b>Pathology</b>: %{x} <br><b>Recurred</b>: %{text} <br><b>Percentage</b>: %{y} <extra></extra>' -->
<!--     ) |> -->
<!--   plotly::config(displayModeBar = FALSE) |> -->
<!--   plotly::layout(bargap = 0.1, barmode = 'stack', -->
<!--                  yaxis = list(title = '', ticksuffix = '%'), -->
