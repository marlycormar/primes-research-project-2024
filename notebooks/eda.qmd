...
```{r eda_set_params, echo=FALSE, cache=TRUE}
...
We will employ three distinct modeling methods to analyze our dataset: Artificial Neural Networks (ANN), K-Nearest Neighbors (KNN), and Support Vector Machines (SVM). Each of these methods brings unique strengths to the analysis, with ANN providing deep learning capabilities, KNN offering simplicity and ease of interpretation, and SVM delivering powerful discriminative classification, thereby encompassing a comprehensive approach to predicting cancer recurrence in the studied dataset.
