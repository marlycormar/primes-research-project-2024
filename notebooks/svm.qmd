...
The Support Vector Machine (SVM) is an efficient approach for classification developed in the 1990s. The SVM generates a hyper-plane or curve that splits the hyper-space into a Yes region (cancer recurs) and a No region (cancer doesn't recur). Given a test observation, its classification is based on the region that it falls in.

Now we explain how SVM works in detail. First, consider the special case in which there is a roughly linear decision boundary between the training observations in one category and those in the other category. In this case, the simpler support vector classifier can be used: generate a hyperplane $\beta_0+\beta_1 x_{i1}+\dots + \beta_p x_{ip}=0$ that mostly separates the two categories by solving the optimization problem of maximizing $M$, the width of the margin, subject to the conditions:
...
Here each $\epsilon_i$ is a small error term, and $C$ is tuning parameter for which larger $C$ corresponds to higher tolerance of violations of the margin. Finally, in the case of a non-linear decision boundary, SVM maps the training data into a higher-dimensional space using kernels. Two common kernels are the polynomial kernel, which maps each observation via a polynomial of degree $d$, and the radial kernel, which maps each observation via its distance from a point. Then the support vector classifier can be applied.
